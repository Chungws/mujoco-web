...
**Status:** Week 3-5 - VLA Server Phase 2 Restructuring (Microservice Architecture)
**Priority:** HIGH
...
- ✅ Episode generation (server-side, up to 50 steps)
- ✅ Browser-based state replay (MuJoCo WASM)
- ✅ Voting system (A/B/Tie/Both Bad)
- ✅ Robot-specific + Global ELO leaderboard
- ✅ Hourly worker aggregation

### What's Deferred (Post-MVP)
- ❌ Multiple robots (Franka, UR5)
...
```
User Flow:
1. Visit /battle → System creates Session
2. System assigns 2 random VLA models (hidden)
3. User provides instruction: "Pick up the red cube"
4. Server executes both models (parallel) → Save episodes to MongoDB
5. Frontend loads states → MuJoCo WASM renders side-by-side
6. User votes (A/B/Tie/Both Bad) → Reveal model identities
7. (Optional) User provides another instruction → repeat from step 3
8. Worker runs hourly → Aggregate votes → Update ELO rankings
```
...
│  │  - Loading state during server execution   │  │
│  │  - Voting interface (after replay)         │  │
│  │  - Model reveal                            │  │
│  └───────────────┬───────────────────────────┘  │
│                  │                                │
│  ┌───────────────▼───────────────────────────┐  │
│  │  Leaderboard Page (/leaderboard)          │  │
│  │  - Franka rankings (robot-specific)       │  │
│  │  - Global rankings                         │  │
...
└────────────────┬─────────────────────────────────┘
    ...
    │  POST /api/battles/{id}/turns      │ → Call VLA Server (HTTP)
    ...
    │                                    │
    │  POST /execute                     │ → Generate episode
    │  ├─ Load MuJoCo (Franka Panda)     │
    │  ├─ Load VLA model (Octo/SmolVLA)  │
    │  ├─ Run episode (max 50 steps)     │
...
- Turn: `user_input` → `instruction`
- Message: DELETE (replaced by MongoDB)
- Vote: Add `robot_id`, `scene_id` (denormalized)
- ModelStats: SPLIT → `ModelStatsByRobot` + `ModelStatsTotal`
...
**See:** [FEATURES/002_VLA_Server.md](./002_VLA_Server.md) for complete specification

**Phase 1: Infrastructure (Complete)** ✅
- [x] Create VLA Server project structure
- [x] Root config/mujoco/ setup (template, robots, scenes)
- [x] XML composition logic (10 tests passing)
- [x] Stateless MuJoCo Environment (16 tests passing)

**Phase 2: VLA Integration (Current - Architecture Restructuring)** 🔄

_Note: Restructuring to microservice architecture for dependency isolation_

- [x] VLA Adapter base class ✅
- [x] Mock adapter (20 tests) ✅
- [ ] **Architecture Restructuring:**
  - [ ] vla-server → vla-server-base (common library)
  - [ ] vla-servers/mock (independent service)
  - [ ] vla-servers/octo-small (Python 3.11, TensorFlow 2.15)
  - [ ] vla-servers/smolvla (Python 3.12, PyTorch 2.9+)
- [ ] Each service with FastAPI app (POST /predict)
- [x] Backend integration (TurnService → HTTP → Model Services) ✅

**Backend APIs (Complete)** ✅
- [x] Turn API (POST /battles/{id}/turns) - TDD with 7 tests
- [x] Episodes API (GET /episodes/{id}) - TDD with 5 tests
- [x] Votes API (POST /api/votes) - TDD with 16 tests

**Frontend (Partial)** 🔄
- [x] Battle page (/battle)
- [ ] MuJoCo WASM integration (state replay) - Placeholder UI only
- [x] Side-by-side viewers (UI structure)
- [x] Voting interface
- [x] Leaderboard page (/leaderboard)

**Worker & Leaderboard (Complete)** ✅
- [x] Worker setup (APScheduler)
- [x] Vote aggregation
- [x] ELO calculation (robot-specific + global)
- [x] Update model_stats tables (ModelStatsByRobot + ModelStatsTotal)
- [x] Leaderboard API (GET /api/leaderboard?robot_id=...)
...
### MVP Complete When:
1. ✅ Users can create battles (1 robot, 1 scene)
2. ✅ System assigns 2 random VLA models (hidden)
3. ✅ Server executes both models (50 steps max)
4. ⏳ Browser replays episodes (MuJoCo WASM state-based) - Placeholder UI only
5. ✅ Users can vote and see model reveal
6. ✅ Leaderboard displays WidowX-specific + Global ELO
7. ✅ Worker aggregates votes hourly
8. ⏳ All tests pass - Backend/Worker tests passing, Frontend needs WASM integration
9. ⏳ Performance targets met - Pending WASM implementation
...
**Status:** Active Development (Week 3-5 - VLA Server Phase 2 In Progress)
